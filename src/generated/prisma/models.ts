
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/Usuario.js'
export type * from './models/Notificacion.js'
export type * from './models/NotificacionUsuario.js'
export type * from './models/Socio.js'
export type * from './models/Postulante.js'
export type * from './models/Cv.js'
export type * from './models/Categoria.js'
export type * from './models/Oferta.js'
export type * from './models/OfertaCategoria.js'
export type * from './models/Postulacion.js'
export type * from './models/ConfiguracionCuota.js'
export type * from './models/AjusteCuotaSocio.js'
export type * from './models/Cuota.js'
export type * from './models/PagoCuota.js'
export type * from './models/PagoCuotaDetalle.js'
export type * from './commonInputTypes.js'